...
# Clean up openModule function completely to support Module 1 and Module 2 explicitly with their custom designs
new_script = """
                ...
                target.innerHTML = `
                    <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 24px; border-bottom: 1px solid var(--border-color); padding-bottom: 15px;">
                        <div>
                            ...
                            <p style="color: var(--text-muted); font-size: 13.5px; margin-top: 4px;">Dedicated Multi-Tenant Execution Engine.</p>
                        </div>
...
# Replace script safely by finding window.openModule
if "window.openModule = " in html:
    parts = html.split("window.openModule = ")
    # Keep up to window.openModule, insert new, and keep remaining after old function
    remaining = parts[1].split("};")[1:] if "};" in parts[1] else [parts[1]]
    html = parts[0] + "window.openModule = " + new_script + "".join(remaining)
else:
    html = html.replace("</script>", new_script + "\n</script>")

with open("index.html", "w", encoding="utf-8") as f:
    f.write(html)

print("✔ Module 2 custom view rendered successfully!")
