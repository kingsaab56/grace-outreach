...
# 1. Favicon ke liye exact 32x32 aur 64x64 standard dimensions generate karein
img = Image.open(img_path)
buf32 = io.BytesIO()
img.resize((32, 32), Image.Resampling.LANCZOS).save(buf32, format="PNG")
fav32_b64 = base64.b64encode(buf32.getvalue()).decode('utf-8')

buf64 = io.BytesIO()
img.resize((64, 64), Image.Resampling.LANCZOS).save(buf64, format="PNG")
fav64_b64 = base64.b64encode(buf64.getvalue()).decode('utf-8')

portal_code = f'''import os
import sys
import base64
...
FAV32_BYTES = base64.b64decode("{fav32_b64}")
FAV32_URI = "data:image/png;base64,{fav32_b64}"
FAV64_URI = "data:image/png;base64,{fav64_b64}"
...
<head>
    ...
    <!-- STANDARD COMPREHENSIVE FAVICON SUITE -->
    <link rel="icon" type="image/png" sizes="32x32" href="{{FAV32_URI}}">
    <link rel="icon" type="image/png" sizes="64x64" href="{{FAV64_URI}}">
    <link rel="shortcut icon" href="/favicon.ico?v=2">
    <link rel="apple-touch-icon" href="{{FAV64_URI}}">
...
<body>
    ...
    <div id="launchSplash">
        <div class="splash-crest"><img src="{{FAV64_URI}}" alt="Grace Crest"></div>
    ...
    <div id="cinematicStage">
        ...
        <div class="hologram-emblem"><img src="{{FAV64_URI}}" alt="Grace Hologram"></div>
    ...
    <div id="authViewport">
        <div class="auth-glass-panel">
            <div class="brand-crest"><img src="{{FAV64_URI}}" alt="Grace Crest"></div>
            ...
            <form onsubmit="handleAuthSubmit(event)" autocomplete="off">
                <div class="form-group">
                    ...
                    <input type="text" class="form-control" placeholder="Enter Colleague ID" required autocomplete="off">
                </div>
                <div class="form-group">
                    <label>Security Keyphrase</label>
                    <input type="password" class="form-control" placeholder="Enter Keyphrase" required autocomplete="new-password">
                </div>
    ...
    <div id="enterpriseApp">
        <header class="top-navbar">
            <div style="display: flex; align-items: center; gap: 14px;">
                <div class="nav-logo-thumb"><img src="{{FAV64_URI}}" alt="Grace Logo"></div>
                <div>
...
class GraceHandler(BaseHTTPRequestHandler):
    def do_GET(self):
        if self.path.startswith('/favicon.ico') or self.path in ['/favicon.png', '/logo.png']:
            self.send_response(200)
            self.send_header('Content-Type', 'image/png')
            self.send_header('Cache-Control', 'no-cache, no-store, must-revalidate')
            self.end_headers()
            self.wfile.write(FAV32_BYTES)
            return
...
print("✔ Favicon integration complete with direct standard PNG headers!")
