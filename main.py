...
def _validate_shared_update(payload):
    ...
    if resource not in {"photos", "attendance", "leaves", "clearedFines"}:
...
LOGO_ASSET = Path(__file__).resolve().parents[3] / "attached_assets" / "Gemini_G_1788371414587.jfif"
FAVICON_DATA_URI = (
...
def render_header():
    ...
    <div class="card top-bar">
        <div style="display:flex; align-items:center;">
            <img src="/api/assets/grace-logo.jfif" class="brand-image" alt="Grace Outreach logo">
            <div>
    ...
    <aside id="ai-assistant" class="ai-drawer" aria-label="Grace AI Guide" aria-hidden="true">
        <div class="ai-drawer-head"><div><span class="eyebrow">MODULE 11 · ONLINE</span><h3>Grace AI Guide</h3><small style="display:block;color:var(--text-muted);margin-top:4px;">Drag the robot anywhere · workflow copilot</small></div><div style="display:flex;align-items:flex-start;gap:8px;"><select id="ai-language" aria-label="Guide language" onchange="setAILanguage(this.value)"><option value="en">English</option><option value="ur">Roman Urdu</option></select><button class="modal-close" onclick="closeAIAssistant()" aria-label="Close AI Guide">×</button></div></div>
...
BASE_CSS = """
    ...
    .robot-3d { position:relative; width:43px; height:48px; display:block; filter:drop-shadow(4px 7px 4px rgba(0,0,0,.32)); }
    ...
    .presence { display:inline-flex; align-items:center; gap:5px; margin-left:auto; color:var(--text-muted); font-size:10px; font-weight:700; }
    ...
    .module-card { min-height: 83px; background: var(--bg-card); border: 1px solid var(--border-color); border-radius: 10px; padding: 12px; text-decoration: none; display: flex; align-items: center; gap: 11px; color: inherit; transition: 0.2s; }
    ...
    .mod-title { font-size: 10px; font-weight: bold; color: var(--accent-gold); margin-bottom: 5px; white-space: nowrap; overflow: hidden; text-overflow: ellipsis; }
    .mod-name { font-size: 12px; font-weight: bold; line-height: 1.25; }
    .module-desc { margin-top: 4px; color: var(--text-muted); font-size: 9px; line-height: 1.25; white-space: nowrap; overflow: hidden; text-overflow: ellipsis; }
    ...
    @media (max-width: 700px) { body { padding: 10px; } .modules-grid { grid-template-columns: repeat(2, minmax(0, 1fr)); } .palette-grid, .soundscape-options, .typography-grid, .form-grid, .color-control-grid { grid-template-columns: repeat(2, 1fr); } .audio-player-shell, .clip-grid { grid-template-columns:1fr; flex-direction:column; align-items:stretch; } .attendance-summary-grid { grid-template-columns:repeat(2,minmax(0,1fr)); } .leave-row { grid-template-columns:1fr 1fr; } .leave-row > div { grid-column:1 / -1; } }
    ...
    @media (max-width: 480px) { .modules-grid { grid-template-columns: 1fr; } .top-bar { align-items: flex-start; } .view-as-bar { align-items:flex-start; flex-direction:column; } .view-as-controls { align-items:flex-start; flex-direction:column; width:100%; } .view-as-controls select { width:100%; min-width:0; } .colleague-grid { grid-template-columns:1fr; } .profile-details { grid-template-columns:1fr; } .permission-grid { grid-template-columns:repeat(3, minmax(0, 1fr)); } .telemetry-grid { grid-template-columns:1fr; } .module-hero { flex-direction:column; } .module-status-pill { align-self:flex-start; } .dispatch-checks { grid-template-columns:1fr; } .ai-mascot { right:14px; bottom:14px; } }
"""
...
const PROFILE_DATA = {
    king: {name:'King Saab', role:'Super Admin', status:'Online', metrics:{pipeline:'2,480', inboxes:'3 Inboxes', volume:'1,240', deal:'$64,800'}},
    abdullah: {name:'Abdullah Khan', role:'Strategic Lead', status:'Online', metrics:{pipeline:'1,860', inboxes:'3 Inboxes', volume:'920', deal:'$48,200'}},
    sarah: {name:'Sarah Malik', role:'Growth Marketer', status:'Online', metrics:{pipeline:'1,120', inboxes:'2 Inboxes', volume:'640', deal:'$18,400'}},
    hamza: {name:'Hamza Ali', role:'Lead Collector', status:'Offline', metrics:{pipeline:'740', inboxes:'1 Inbox', volume:'410', deal:'$12,600'}}
};
...
const MODULE_GUIDES = {
    ...
    10:{name:'Audio Studio & Extractor',en:'♫ Step 1 ➔ Choose an ambient track or upload media.\\nStep 2 ➔ Set clip start and end points.\\nStep 3 ➔ Test the alert chime or open Broadcast Center.',ur:'♫ Step 1 ➔ Soundscape kholen, track select karein ya local audio/video load karein. Start aur end seconds set karke clip apply karein.'},
...
function persistProfilePhoto(key, file, onDone) {
    ...
    if (file.size > 2200000) { showToast('Profile images must be smaller than 2 MB.', 'warning'); return; }
    const reader = new FileReader();
    reader.onload = function() {
        try {
            const photos = JSON.parse(window.localStorage.getItem('grace-profile-photos') || '{}');
            ...
            showToast('Profile picture saved for future visits.', 'success');
        } catch (error) {
...
function openColleagueProfile(key, name, softwareId, role, tags, initials) {
    ...
    if (!modal || !body) return;
    document.getElementById('profile-title').innerText = name + ' · Profile & Access';
    body.innerHTML = '<div class="profile-details"><div><div id="profile-photo-preview" class="profile-photo" data-profile-avatar="' + key + '">' + initials + '</div><label class="upload-zone profile-upload"><span>＋ Upload profile picture</span><small>Saved locally and restored after refresh.</small><input type="file" accept="image/*" onchange="previewProfilePhoto(event, &quot;' + key + '&quot;)"></label></div><div class="profile-summary"><strong>' + name + '</strong><small>' + role + '</small><small>Software ID · <b>' + softwareId + '</b></small><div class="tag-list">' + tags.split(',').map(function(tag) { return '<span class="tag">' + tag + '</span>'; }).join('') + '</div><div class="permission-card"><div class="permission-card-head"><strong>Access summary</strong><small>Managed by Super Admin</small></div><p style="margin:0;color:var(--text-muted);font-size:11px;line-height:1.5;">Permissions are controlled from the RBAC matrix on the colleague card. Use View-As to preview the exact restricted workspace.</p></div></div></div>';
    modal.hidden = false;
    try {
        const photos = JSON.parse(window.localStorage.getItem('grace-profile-photos') || '{}');
    ...
    <main id="module-workspace" data-module-page-id="{m_id}">
        <div class="module-authorized-content">
            <div class="card module-hero">
                ...
                <div style="display:grid;justify-items:end;gap:10px;"><span class="module-status-pill"><i class="presence-dot online"></i>{mod_info["status"]}</span><a href="/api/?tab=matrix" class="btn btn-blue module-back-button">← Back to Main Matrix</a></div>
            </div>
    ...
    for colleague in colleagues:
        ...
        <article class="colleague-card">
            <div class="colleague-head">
                ...
                <div><div class="colleague-name">{colleague["name"]}</div><div class="colleague-role">{colleague["role"]}</div></div>
                <span class="presence"><i class="presence-dot {online_class}"></i>{colleague["status"]}</span>
            </div>
            ...
            <div class="colleague-actions"><button class="btn btn-blue" onclick="openColleagueProfile('{colleague["key"]}', '{colleague["name"]}', '{colleague["software_id"]}', '{colleague["role"]}', '{",".join(colleague["tags"])}', '{colleague["initials"]}')">View profile</button><button class="btn btn-gray" onclick="changeViewAs('{colleague["key"]}')">View as</button></div>
